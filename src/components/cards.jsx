    ...
    return (
      <Link href={`#/course/${c.id}`} className="card card-lift card-lift group flex flex-col gap-2.5 p-5 no-underline transition-transform duration-150 hover:-translate-y-[3px]">
        <div className="flex items-center justify-between">
          <div className="flex h-[44px] w-[44px] items-center justify-center rounded-[4px] bg-tintviolet text-violet"><Icon name={c.icon} size={22} /></div>
      ...
      {s.decks.map((d) => {
        ...
        return (
          <Link key={d.id} href={`#/slide/${cid}/${d.id}/1`} className="card card-lift card-lift flex gap-3.5 p-3 no-underline transition-transform duration-150 hover:-translate-y-0.5">
            {first ? (
              <div className="h-[76px] w-[132px] shrink-0 overflow-hidden rounded-[8px] border border-borderline bg-hover">
                <img loading="lazy" src={first} alt="" className="block h-full w-full object-cover" />
              </div>
            ) : (
              <div className="flex h-[76px] w-[132px] shrink-0 items-center justify-center overflow-hidden rounded-[8px] border border-borderline bg-hover text-textmuted opacity-60">
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(240px,1fr))] gap-3.5">
        {resources.map((r, i) => (
          <a key={i} href={r.url} target="_blank" rel="noopener noreferrer" className="card card-lift card-lift flex gap-3.5 p-3 no-underline transition-transform duration-150 hover:-translate-y-0.5">
            <div className="flex h-[76px] w-[132px] shrink-0 items-center justify-center rounded-[8px] border border-borderline bg-hover text-textmuted opacity-60">
