  ...
  return (
    <div className="container mx-auto max-w-[1080px]">
      <div className="mb-6 rounded-[16px] border border-borderline bg-gradient-to-br from-blue/20 via-cyan/10 p-6 backdrop-blur-2xl max-sm:p-4">
        <h1 className="mb-1 text-2xl font-semibold max-sm:text-xl">Quiz de révision</h1>
      ...
      <div className="grid grid-cols-2 gap-4 max-lg:grid-cols-2 max-sm:grid-cols-1">
        {items.map((q) => (
          <Link key={q.courseId} href={`#/quiz/${q.courseId}`} className="group no-underline">
            <Card className="flex h-full items-center gap-4 p-5 transition-colors group-hover:border-cyan/40 max-sm:p-4">
              <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-[12px] bg-cyan/15 text-cyan">
                <Icon name={q.icon} size={22} />
              </span>
              <div className="min-w-0 flex-1">
                <div className="truncate text-[15px] font-bold text-textmain">{q.title}</div>
                <div className="mt-1 flex flex-wrap items-center gap-1.5">
                  <Badge color="blue">{q.questions.length} questions</Badge>
