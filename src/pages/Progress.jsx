  ...
  return (
    <div className="ambient container mx-auto max-w-[1080px]">
      ...
      <Card className="mb-6 p-6 max-sm:p-4">
        ...
        {next ? (
          <Link href={`#/lesson/${next.course.id}/${next.index}`} className="mt-5 inline-flex items-center gap-2 rounded-[10px] bg-green px-4 py-2.5 text-sm font-bold text-onaccent no-underline transition-opacity hover:opacity-90">
          ...
          return (
            <Link key={c.id} href={`#/course/${c.id}`} className="no-underline">
              <Card className="flex flex-wrap items-center gap-4 p-4.5 transition-colors hover:bg-hover max-sm:p-3.5">
                ...
                <span className={`rounded-full px-2.5 py-1 text-[11px] font-bold ${read === total ? "bg-tintgreen text-green" : read > 0 ? "bg-tintviolet text-violet" : "bg-hover text-textmuted"}`}>
